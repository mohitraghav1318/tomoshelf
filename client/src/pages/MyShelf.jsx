...
const MyShelf = () => {
    ...
    if (loading) {
        return (
            <div className="min-h-screen pt-32 px-6 bg-black">
    ...
    return (
        <div className="min-h-screen bg-black font-sans text-slate-200 pt-32 pb-12 px-6">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
                    {[
                        ...
                        { label: 'Reading', val: allBooks.filter(e => e.status === 'reading').length, color: 'text-red-500' },
                        { label: 'Completed', val: allBooks.filter(e => e.status === 'completed').length, color: 'text-green-500' },
                        { label: 'Want to Read', val: allBooks.filter(e => e.status === 'want-to-read').length, color: 'text-red-500' }
                    ].map((s, i) => (
                        <div key={i} className="bg-neutral-950 border border-slate-800 rounded-2xl p-6 shadow-lg">
                    ...
                    {[
                        ...
                        <button
                            ...
                            className={`px-6 py-2 rounded-full text-xs font-bold tracking-widest transition-all ${
                                ...
                                : 'bg-neutral-950 text-slate-500 border border-slate-800 hover:bg-slate-800'
                            }`}
                        >
                ...
                {/* Shelf Grid */}
                {shelf.length === 0 ? (
                    <div className="text-center py-32 bg-neutral-950/30 rounded-3xl border border-slate-900/50">
                        ...
                        <button
                            onClick={() => navigate('/')}
                            className="bg-red-600 hover:bg-red-700 text-white px-8 py-3 rounded-xl font-bold transition-all shadow-lg shadow-red-900/40"
                        >
                            Discover Books
                        </button>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {shelf.map((entry) => (
                            <div key={entry._id} className="bg-neutral-950 border border-slate-800 rounded-2xl overflow-hidden hover:border-red-500/50 transition-all shadow-xl group">
                                <div className="flex gap-6 p-5">
                                    ...
                                    <div className="flex-1 min-w-0 flex flex-col justify-between">
                                        <div>
                                            <h3 className="font-bold text-white text-sm line-clamp-2 mb-1 hover:text-red-400 cursor-pointer transition-colors" onClick={() => navigate(`/book/${entry.bookId}`)}>
                                            ...
                                            <select
                                                ...
                                                className="w-full bg-slate-800 border-none rounded-lg py-1.5 px-3 text-[10px] font-bold text-white uppercase tracking-widest cursor-pointer focus:ring-1 focus:ring-red-500 focus:outline-none"
                                            >
                                        ...
                                        <div className="flex items-center justify-between pt-4 mt-auto">
                                            ...
                                            {entry.status === 'reading' && (
                                                <button 
                                                    ...
                                                    className="flex items-center gap-1.5 px-3 py-1 bg-red-600/10 text-red-500 rounded-lg text-[10px] font-bold uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all"
                                                >
